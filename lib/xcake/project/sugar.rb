  ...
  class Project
    ...
    def unit_tests_for(host_target, &block)
      target do |t|
        ...
        t.all_configurations.each {|c| c.settings["TEST_HOST"] = "$(BUILT_PRODUCTS_DIR)/#{host_target.name}.app/#{host_target.name}"}
        t.all_configurations.each {|c| c.settings["BUNDLE_LOADER"] = "$(TEST_HOST)"}
